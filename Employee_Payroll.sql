...
SELECT ED.NAME,BASIC_PAY FROM EMPLOYEES_DETAILS ED INNER JOIN EMP_SALARY ES ON ED.EMP_ID = ES.EMP_ID WHERE ED.NAME = 'Nantha'
...
--CREATING MT_EMPLOYEE_DETAILS TABLE
CREATE TABLE MT_EMPLOYEES_DETAILS(
EMP_ID INT PRIMARY KEY NOT NULL,
NAME VARCHAR(50) NOT NULL,
GENDER VARCHAR(2) NOT NULL,
PHONE FLOAT NOT NULL,
EMAIL VARCHAR(50) NOT NULL)
ALTER TABLE MT_EMPLOYEES_DETAILS ADD START_DATE DATE NOT NULL
ALTER TABLE MT_EMPLOYEES_DETAILS ADD COMPANY_ID INT NOT NULL

DROP TABLE MT_EMPLOYEES_DETAILS

--CREATING EMPLOYEE_SALARY TABLE
CREATE TABLE MT_EMP_SALARY(
EMP_ID INT PRIMARY KEY NOT NULL,
BASIC_PAY FLOAT NOT NULL,
DEDUCTIONS FLOAT NOT NULL,
TAXABLE_PAY FLOAT NOT NULL,
...
GO
CREATE OR ALTER PROCEDURE [dbo].[Sp_InsertEmployeeDetailsAndSalary_MultiThreading]
(@COMPANY_ID INT,
@EMP_ID INT,
@NAME VARCHAR(50),
@GENDER VARCHAR(2),
...
AS
BEGIN TRY
IF NOT EXISTS (SELECT EMP_ID FROM MT_EMPLOYEES_DETAILS WHERE EMP_ID = @EMP_ID)
BEGIN
INSERT INTO MT_EMPLOYEES_DETAILS VALUES(@EMP_ID,@NAME,@GENDER,@PHONE,@EMAIL,@START_DATE,@COMPANY_ID)
END
IF NOT EXISTS (SELECT EMP_ID FROM MT_EMP_SALARY WHERE EMP_ID = @EMP_ID)
BEGIN
INSERT INTO MT_EMP_SALARY VALUES(@EMP_ID,@BASIC_PAY,@DEDUCTIONS,@TAXABLE_PAY,@INCOME_TAX,@NET_PAY)
END
END TRY
BEGIN CATCH
PRINT ERROR_MESSAGE()
...
SELECT * FROM MT_EMPLOYEES_DETAILS
SELECT * FROM MT_EMP_SALARY

ALTER TABLE MT_EMP_SALARY ADD CONSTRAINT FK_EMP_ID_FOR_MT_EMP_SALARY FOREIGN KEY(EMP_ID) REFERENCES MT_EMPLOYEES_DETAILS(EMP_ID) ON DELETE CASCADE ON UPDATE CASCADE

GO
CREATE OR ALTER PROCEDURE [dbo].[Sp_Update_MT_EMPLOYEES_DETAILS_AND_EMP_SALARY]
(@EMP_ID INT,
@BASIC_PAY FLOAT)
AS
BEGIN TRY
UPDATE MT_EMP_SALARY SET BASIC_PAY = @BASIC_PAY WHERE EMP_ID = @EMP_ID
END TRY
BEGIN CATCH
PRINT ERROR_MESSAGE()
END CATCH